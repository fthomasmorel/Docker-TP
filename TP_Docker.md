...
Un container Docker est la brique de base. C'est une instance qui permet d'isoler une partie des ressources du système. Comme vue en cours, un container est différent d'une machine virtuelle de part son architecture avec le système. 
...
1. Lancez un shell interactif à partir d'une image Ubuntu sur un container et "bind" le port 80 du container sur le port 9001 du host. (cf. Cours)
2. Installez un Apache ou un Nginx ou autre selon votre choix
...
Créez un répertoire ```nginx```. Dans ce répertoire, créez un fichier HTML (```test.html``` par exemple) et ajoutez-y du code (faites pas un truc qui clignotte, ca fait mal aux yeux 😎). Depuis le répertoire ```nginx```, lancez un container "nginx" avec l'option ```-v```, tel que :
...
Il rendra alors le message déchiffré sur la sortie standard. Donnez ce message au responsable de TP pour accéder à la suite.
...
RubyChat est une application en passe de rivaliser avec les plus grandes applications _moderne_ de messageries instantannées telles que AIM, MSN ou ICQ.
...
De l'autre côté, une partie front-end, écrite en AngularJS, permet d'afficher l'ensemble des messages ainsi que d'en envoyer.

L'application est en train de grimper dans les charts, et l'équipe de développeurs commence à penser au futur. 

- Comment rendre l'application RubyChat scalable pour supporter la charge des futurs 100 000 000 d'utilisateurs ? 
...
L'application web, quant à elle, a été ecrite avec le framework AngularJS (v1). Voici l'architecture du répertoire RubyChat-Front :
...
Le serveur web ```nginx``` permet de servir les fichiers de la partie web. Aussi, il redirige les requêtes qui arrivent sur ```/api``` vers l'application ```ruby```. Cette dernière communique avec la base de données ```mongodb```.

## Docker-compose

Afin de répondre aux problématiques posées par les developpeurs de RubyChat, vous allez devoir dockeriser les différents modules de l'application. Le but de l'exercice est de voir comment on peut monter une architecture telle que celle de RubyChat en utilsant des containers.
...
- Définir un point d'entré pour exécuter ```rubychat.rb```
...
Pour le container ```nginx```, nous allons partir de l'image docker de base ```nginx```. Dans le répertoire ```RubyChat-Front```, créez un fichier ```Dockerfile```. Puis, vous devez :
...
```
Pour le moment, nginx ne connait pas encore le host pour joindre l'API ruby, il ne vous ait donc pas possible de lancer le container pour l'instant.

### mongoDB

Pour le container ```mongoDB```, rien de plus simple. Il suffit d'utiliser l'image docker de base ```mongodb```. Elle vous permettra de faire tourner une instance de mongoDB facilement.

### Configurer docker-compose

```docker-compose``` a besoin de deux fichiers pour être configuré. Un fichier ```docker-compose.yml``` qui permet de définir les containers et leur liens, et un fichier ```requirements.txt``` qui permet de lister les images nécessaires à l'architecture définit dans le ```docker-compose.yml```.
...
Dans le fichier ```docker-compose.yml```, complétez le squelette suivant et supprimez les commentaires :
